...
function initThemeToggle() {
  ...
  function renderTheme(theme) {
    if (theme === "dark") {
      ...
      themeToggleBtn.setAttribute("title", "Zu hellem Design wechseln");
    } else {
      document.documentElement.removeAttribute("data-theme");
      themeToggleBtn.innerHTML = moonIconSvg;
      themeToggleBtn.setAttribute("aria-label", "Zu dunklem Design wechseln");
      themeToggleBtn.setAttribute("title", "Zu dunklem Design wechseln");
    }
  }
...
function initNavigation() {
  ...
  // Handle initial view based on hash or default to notes
  const rawHash = window.location.hash.replace("#", "");
  const initialTarget = rawHash && document.getElementById(`view-${rawHash}`) ? rawHash : "notes";
